...
export default function ObjektePageTablet({ navS, navI: _navI }: { navS: string | null; navI: string | null }) {
  ...
  useEffect(() => {
    ...
    const sections = works.map((_, i) => document.getElementById(`section-${i}`));
      ...
      (entries) => {
        ...
        if (visible) {
          ...
          if (idx !== -1) {
            ...
            setActiveInfo({ title: work.title, year: work.year, size: work.size, index: Math.min(activeIdx, total - 1) + 1, total });
          }
        }
      },
      { threshold: 0.5 }
  ...
  useEffect(() => {
    ...
    setActiveInfo({ title: first.title, year: first.year, size: first.size, index: total > 0 ? 1 : 0, total });
  }, [works]);
          ...
          <section id={`section-${wIdx}`} key={wIdx} className="arbeiten-section">
            ...
            <Swiper
              ...
              onSlideChange={(swiper) => {
                ...
                setActiveIndexes((prev) => { const arr = [...prev]; arr[wIdx] = nextIdx; return arr; });
                setActiveInfo({ title: work.title, year: work.year, size: work.size, index: nextIdx + 1, total });
              }}
            >
                ...
                <SwiperSlide key={i}>
                  <div className="swiper-zoom-container">
                    <img
                      ...
                      onLoad={() => {
                        if (i === activeIdx) {
                          setActiveInfo({ title: work.title, year: work.year, size: work.size, index: i + 1, total });
                        }
                      }}
                    />
                  </div>
