...
export default function SerienPageTablet({ navS, navI }: { navS: string | null; navI: string | null }) {
  ...
  useEffect(() => { setActiveSection(activeSerie); }, [setActiveSection, activeSerie]);

  useEffect(() => {
    const sections = SERIEN.map((_, i) => document.getElementById(`section-${i}`));
        ...
        if (visible) {
          ...
          if (idx !== -1 && idx !== activeSerie) {
            ...
            if (!gridView[idx] && work) {
              setActiveInfo({
                title: work.piece != null && work.piece !== "" ? String(work.piece) : String(serie.series ?? ""),
  ...
  useEffect(() => {
    ...
    setGridView((prev) => { const next = [...prev]; next[s] = false; return next; });
    setImgIdxBySerie((prev) => { const next = [...prev]; next[s] = i; return next; });
    requestAnimationFrame(() => {
      document.getElementById(`section-${s}`)?.scrollIntoView({ behavior: "auto", block: "start" });
  ...
  const selectImage = (sIdx: number, workIndex: number) => {
    setImgIdxBySerie((prev) => { const next = [...prev]; next[sIdx] = workIndex; return next; });
    setGridView((prev) => { const next = [...prev]; next[sIdx] = false; return next; });
    ...
    document.getElementById(`section-${sIdx}`)?.scrollIntoView({ behavior: "smooth", block: "start" });
  };

  const backToGrid = (sIdx: number) => {
    setGridView((prev) => { const next = [...prev]; next[sIdx] = true; return next; });
    setActiveInfo(null);
  };

  const handleSlideChange = (sIdx: number, nextIdx: number) => {
    setImgIdxBySerie((prev) => { const next = [...prev]; next[sIdx] = nextIdx; return next; });
    const serie = SERIEN[sIdx];
  ...
  return (
    <main className="arbeiten-main mobile-view-only serien-main">
      {SERIEN.map((serie, sIdx) => {
        ...
        return (
          <section id={`section-${sIdx}`} key={sIdx} className="arbeiten-section serien-section">
            <header className="technik-title-sticky">
              <h2>{String(serie.series ?? "")}</h2>
              {serie.year && <div className="serien-year">{String(serie.year)}</div>}
            </header>

            {gridView[sIdx] ? (
              <div className="grid-container serien-grid">
                {allImages.map((imgObj, idx) => (
                  <div key={idx} className="grid-item" onClick={() => selectImage(sIdx, imgObj.workIndex)}>
                    <img
                      src={imgObj.thumb}
              ...
              <Swiper
                ...
                onSlideChange={(swiper) => handleSlideChange(sIdx, swiper.activeIndex)}
              >
                {works.map((w, wi) => {
                  ...
                  return (
                    <SwiperSlide key={wi}>
                      <div className="swiper-zoom-container">
                        <img
                          src={firstImg}
                          alt={(w.piece != null && String(w.piece)) || String(serie.series)}
      ...
      <style jsx>{`
        ...
        .tablet-swiper :global(.swiper-wrapper) { width: 100%; height: 100%; }
        .tablet-swiper :global(.swiper-slide) {
          display: flex;
          align-items: center;
          justify-content: center;
        }
        .tablet-swiper :global(.swiper-zoom-container) {
          display: flex;
          align-items: center;
          justify-content: center;
        }
        .serien-year { font-size: 0.85rem; margin-top: 4px; color: #555; }
      `}</style>
    </main>
  );
}
